...
{
  private static int SolveLine(string input)
  {
    char? first = null;
    char? last = null;
    ...
    {
      throw new NullReferenceException("No digits were found");
    }
  ...
  public static void TestSolveLine()
  {
    ...
    {
      var result = SolveLine(input.Value);
      if (result == input.Key)
      {
      ...
      {
        Console.WriteLine($"Test failed - expected {input.Key} got {result} for {input.Value}");
      }
    }
  }
...
{
  private class Lexer(string input)
  {
    ...
    private static char GetChar(string input) => input switch
    {
      "one" => '1',
      "two" => '2',
      "three" => '3',
    ...
    {
      var charsRemaining = Input.Length - curr;
      string? subst;
      if (charsRemaining >= 5)
      {
        subst = Input.Substring(curr, 5);
        if (subst == "seven" || subst == "eight" || subst == "three")
        {
          curr += 3;
          next += 3;
          return GetChar(subst);
        }
      }

      if (charsRemaining >= 4)
      {
        subst = Input.Substring(curr, 4);
        if (subst == "four" || subst == "five" || subst == "nine")
        {
          curr += 2;
          next += 2;
          return GetChar(subst);
      ...
      if (charsRemaining >= 3)
      {
        subst = Input.Substring(curr, 3);
        if (subst == "one" || subst == "two" || subst == "six")
        {
          curr += 1;
          next += 1;
          return GetChar(subst);
  ...
  public static void TestSolveLine()
  {
    Dictionary<int, string> inputs = new()
    {
      {29, "two1nine"},
      {83, "eightwothree"},
      {13, "abcone2threexyz"},
